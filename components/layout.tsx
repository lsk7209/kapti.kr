  ...
  return (
    <Section title={copy.title} note="출처·검증·다음 행동">
      <div className="guidecard" style={{ display: "block" }}>
        ...
        <p>
          예를 들어 같은 시군구 안에서 한 단지의 공용관리비가 높게 보인다면 바로 비효율이라고 판단하지
          않습니다. 경비 인력 배치, 청소 범위, 승강기 대수, 지하주차장 규모, 조경 면적, 커뮤니티 시설,
          노후 설비, 외주 계약 조건이 모두 영향을 줄 수 있습니다. 이때 사용자가 해야 할 일은 같은 조건의
          단지를 더 찾아보고, 세부 항목 중 어느 부분이 차이를 만드는지 확인하는 것입니다.
        </p>
        <p>
          반대로 관리비가 낮게 보이는 단지도 무조건 유리하다고 보기 어렵습니다. 필요한 수선이 미뤄졌거나,
          장기수선충당금 적립이 낮거나, 공용시설 운영 범위가 다른 경우라면 단기 금액만으로 판단할 수
          없습니다. 낮은 금액은 좋은 신호일 수 있지만, 왜 낮은지 설명할 수 있을 때 더 유용한 정보가 됩니다.
        </p>
        <p>
          사용자가 실제로 비교할 때는 페이지를 세 개 이상 함께 열어 보는 방법이 좋습니다. 단지 상세에서는
          항목별 숫자를 보고, 지역 상세에서는 또래 분포를 보고, 가이드에서는 항목의 의미를 확인합니다.
          그다음 공식 원천 페이지에서 기준월과 공개 항목을 다시 확인하면 과장된 결론을 줄일 수 있습니다.
        </p>
        <p>
          케이아파티의 문서는 이런 순서를 반복해서 안내합니다. 검색 페이지는 출발점이고, 지역 허브는 비교군을
          좁히는 중간 단계이며, 가이드와 전문 문서는 해석 기준을 정리하는 참고 자료입니다. 사용자가 문의나
          정정을 보낼 때도 단지명, 기준월, 항목명, 원천 링크를 함께 남기면 더 정확하게 검토할 수 있습니다.
        </p>
